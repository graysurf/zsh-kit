...

{
  ...
  assert_contains "$launcher_args" "--supports output-json" "create should probe launcher capabilities" || fail "$launcher_args"
	  ...
	  assert_not_contains "$launcher_args" "--persist-gh-token" "create should not persist GH_TOKEN into container env" || fail "$launcher_args"
	  assert_not_contains "$launcher_args" "--setup-git" "create should not use launcher --setup-git (avoids GH_TOKEN env persistence)" || fail "$launcher_args"
